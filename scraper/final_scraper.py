            ...
            try:
                ...
                if size_match and price_match:
                    ...
                    if size_element and price_element:
                        ...
                        size = re.search(r'(\d+\s*Yard)', size_text)
                        price = re.search(r'(\$\d+(?:\.\d+)?)', price_text)
                        
                        if size and price:
                            print(f"Found dumpster: {size.group(1)} - {price.group(1)}")
                            dumpster_data.append({
                                "dumpster_size": size.group(1),
                                "price": price.group(1)
                            })
            except Exception as e:
...
async def main():
    ...
    sample_df = df.head(10)  # Process first 10 addresses for testing
