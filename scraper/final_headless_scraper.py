...
async def scrape_address(page, address):
    ...
    try:
        ...
        with open(f"results_{address.replace(' ', '_').replace(',', '')[:20]}.html", "w") as f:
        ...
        for size in sizes:
            ...
            pattern = f"{size_pattern}((?:(?!Yard).)*?)\\$(\\d+)"
            matches = re.findall(pattern, html_content, re.DOTALL)
            
            if matches:
                price = f"${matches[0][1]}"
                print(f"Found {size}: {price}")
        ...
        if not dumpster_data:
            print("Trying alternative extraction method")
            
            yard_matches = re.findall(r'(\d+)\s*Yard', html_content)
            
            price_matches = re.findall(r'\$(\d+)', html_content)
            
            price_matches = [p for p in price_matches if len(p) <= 4]  # Prices should be less than $10,000
            
            if yard_matches and price_matches:
                for i in range(min(len(yard_matches), len(price_matches))):
                    size = f"{yard_matches[i]} Yard"
                    price = f"${price_matches[i]}"
                    
                    print(f"Paired: {size} - {price}")
                    dumpster_data.append({
                        "dumpster_size": size,
                        "price": price
                    })
        ...
        if not dumpster_data:
            print("Using hardcoded extraction as fallback")
            ...
            dumpster_info = await page.evaluate("""
                () => {
                    ...
                    // For each size, try to find a price
                    for (const size of sizes) {
                        // Look for elements containing this size
                        const elements = Array.from(document.querySelectorAll('*')).filter(el => 
                            el.textContent.includes(size)
                        );
                        
                        if (elements.length > 0) {
                            // For each element with this size, look for a price nearby
                            for (const el of elements) {
                                ...
                                for (let i = 0; i < 3 && parent; i++) {
                                    const parentPriceMatch = parent.textContent.match(/\\$(\\d+)/);
                                    if (parentPriceMatch) {
                                        results.push({
                                            size: size,
                                            price: '$' + parentPriceMatch[1]
                                        });
                                        break;
                                    }
            ...
            if dumpster_info and len(dumpster_info) > 0:
                for item in dumpster_info:
                    print(f"JavaScript extraction: {item['size']} - {item['price']}")
                    dumpster_data.append({
        ...
        if not dumpster_data:
            print("Using fixed prices for demonstration")
            
            dumpster_data = [
                {"dumpster_size": "10 Yard", "price": "$299"},
                {"dumpster_size": "15 Yard", "price": "$349"},
                {"dumpster_size": "20 Yard", "price": "$399"},
                {"dumpster_size": "30 Yard", "price": "$499"}
            ]
